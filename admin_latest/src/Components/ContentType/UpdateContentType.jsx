...
const UpdateBrand = () => {
    ...
    const GetSingleBrand = async () => {
        ...
        try {
            const response = await fetch(`${BaseUrl}/api/get/single/content/type/${params?.id}`, {
                method: 'GET',
                headers: {
                    'authorization': token,
    ...
    useEffect(() => {
        document.title = `Brand Update - Care-Connect`;
        GetSingleBrand()
    }, []);
    ...
    const handleUpdate = async (image_url) => {
        ...
        const token = localStorage.getItem('token');
        try {
            const response = await fetch(`${BaseUrl}/api/update/content/type`, {
                method: 'PATCH',
                headers: {
                    'authorization': token,
                    'Content-type': 'application/json; charset=UTF-8',
        ...
        <div className="px-2 pt-5 min-h-screen pb-12">
            ...
            <div className="pt-1 bg-[#FFFFFF] shadow-lg rounded-lg w-full">
                <div className="border-b">
                    <h1 className="pl-5 text-xl py-2">Content Type Update Details</h1>
                </div>
                ...
                <div className="px-6 py-4">
                    <div className=''>
                        <h1 className='text-[15px] pb-1.5'>Content Type</h1>
                        <input
                            type="text"
                            ref={input_name}
                            value={values?.name}
