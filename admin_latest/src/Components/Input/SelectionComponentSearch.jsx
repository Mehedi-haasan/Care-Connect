...
const SelectionComponentSearch = ({ options, onSelect, label, className = 'rounded', default_select = false, default_value = "Select a filter" }) => {
  ...
  return (
    <div className='w-full bg-white dark:bg-[#040404] dark:text-white'>
      ...
      <div className={`${hide ? 'border-t border-x pr-[1px]' : 'border '}  rounded-l relative bg-white dark:bg-[#040404] dark:text-white h-[39px] z-10`}>
        ...
        <div className={`font-thin px-1 pt-2 cursor-pointer ${select === "Select a filter" ? 'text-[#6B7280] dark:text-white' : 'text-black dark:text-white'} z-0 text-[12px]`} onClick={() => { setHide(!hide) }}>{default_value}</div>
        <div className={` ${hide ? '' : 'hidden'} absolute left-[-1px] right-[-1px] border-x border-b rounded-b bg-white dark:bg-[#040404]`}>
          <div className='px-2'>
            <input type='text'
              ...
              className='border rounded-l focus:outline-none w-full px-2 py-1 font-thin text-sm dark:bg-[#040404] dark:text-white' onChange={handleFilter} />
          </div>
