...
const Setting = ({ userinfo = { role: 'superadmin' } }) => {
    ...
    useEffect(() => {
        AppInfo()
        document.title = `${select} Setting - Care-Connect`;
    }, [select, compId]);
                    ...
                    <div >
                        ...
                        <div className='bg-[#FFFFFF] dark:bg-[#040404] dark:text-white rounded-b shadow'>
                            ...
                            <div className="px-3 md:px-4 lg:px-5 pb-3 md:pb-4 lg:pb-5">
                                ...
                                <InputComponent label={'Invoice Prefix'} placeholder={user?.shopcode} onChange={(v) => { setUser({ ...user, shopcode: v }) }} />
                                <InputComponent label={'Description'} onChange={(v) => { setUser({ ...user, description: v }) }} />
                                <InputComponent label={'Employee'} onChange={(v) => { setUser({ ...user, description: v }) }} />
                                <div className='py-3'>
                                    <Button onClick={() => { image_url !== null ? handleUploadUpdate() : UpdateSetting(user?.image_url, "") }}
                                        isDisable={false} name={'Update'} />
