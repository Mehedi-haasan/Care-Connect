...
const CreateDistrict = () => {
    ...
    const [values, setValues] = useState({ name: "", division_id: null });
    ...
    const handleCreate = async () => {
        ...
        try {
            const response = await fetch(`${BaseUrl}/api/create/districts`, {
                method: 'POST',
                headers: {
                    'authorization': token,
    ...
    const GetDivisions = async () => {
        const token = localStorage.getItem('token')
        const response = await fetch(`${BaseUrl}/api/get/just/division`, {
            method: 'GET',
            headers: {
                "authorization": token,
                'Content-type': 'application/json; charset=UTF-8',
